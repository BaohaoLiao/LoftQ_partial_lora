...
def quantize_and_save():
    ...
    if any(name in args.model_name_or_path.lower() for name in ["llama", "mistral", "falcon"]):
        ...
        target_modules = ["query_proj", "value_proj"]  # embeddings not supported by peft
    else:
    ...
    with torch.no_grad():
        ...
        for name, m in lora_model.named_modules():
            ...
            elif isinstance(m, torch.nn.Linear) and ("base_layer" not in name) and ("lora_" not in name):
                hooks.append(
                    m.register_forward_hook(
